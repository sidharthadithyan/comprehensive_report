...
for u in range(5):
    for v in range(5):
        ...
        for i in range(1,n_trial+1):
            loadData[i-1,:,:]=np.load(str(odorNo)+"_"+str(plumeNo)+"/data_4_"+str(odorNo)+"_"+str(plumeNo)+"_"+str(i)+".npy")
        ...
        #Saving firing rates
        np.savetxt('stateAveraged_'+str(odorNo)+'_'+str(plumeNo)+'.csv',temp2Matx*1000,delimiter=',')
        np.save(f'stateAveraged_'+str(odorNo)+'_'+str(plumeNo),temp2Matx*1000)
        ...
        plt.plot(t[0:13000],temp2Matx[:,44]*1000)